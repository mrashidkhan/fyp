<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>MAJU FYP Projects</title>
    <style>
        .navbar-nav li a {
            color: #D62C1F;
        }

        .dropdown-menu>li>a {
            color: blue !important;
        }
    </style>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
</head>
<body class="mt-4">
    <div class="container-fluid">
        <header>
            <div class="container text-center">
                <div class="row">

                    <div class="col-2">
                        <div class="logo">
                            <img src="maju.png" alt="MAJU Logo">
                            <h6 style="color:red ">Live, Learn and Be Inspired</h6>
                        </div>
                    </div>
                    <div class="col-10 mt-5">
                        <h3 style="color:blue">Mohammad Ali Jinnah University</h3>
                        <h4 style="color:red">Final Year Projects</h4>
                    </div>

                </div>
        </header>
    </div>
    <nav class="navbar navbar-expand-lg navbar-light bg-light mt-4">
        <div class="container">
            <!-- <a class="navbar-brand" href="www.jinnah.edu">Home</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button> -->
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            FACULTY OF BUSINESS ADMINISTRATION
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="#">SCHOOL OF BUSINESS ADMINISTRATION</a></li>
                            <li><a class="dropdown-item" href="#">DEPARTMENT OF MANAGEMENT SCIENCES</a></li>

                        </ul>
                    </li>
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            FACULTY OF COMPUTING
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="#">COMPUTER SCIENCE</a></li>
                        </ul>
                    </li>

                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            FACULTY OF ENGINEERING
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="#">ELECTRICAL AND COMPUTER ENGINEERING</a></li>
                        </ul>
                    </li>

                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            FACULTY OF LIFE SCIENCES
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="#">BIOSCIENCES</a></li>
                        </ul>
                    </li>

                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                            data-bs-toggle="dropdown" aria-expanded="false">
                            FACULTY OF SOCIAL AND BASIC SCIENCES
                        </a>
                        <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                            <li><a class="dropdown-item" href="#">ECONOMICS</a></li>
                            <li><a class="dropdown-item" href="#">PSYCHOLOGY</a></li>
                            <li><a class="dropdown-item" href="#">BASIC SCIENCES</a></li>
                        </ul>
                    </li>

                </ul>
            </div>
        </div>
    </nav>

    <div class="container-fluid mt-5">
  <header>
    <div class="container text-center">
      <div class="row">
        
        <?php
          $faculties = array(
            array(
              'name' => 'Faculty of Business Administration',
              'description' => 'Some quick example text to build on the card title and make up the bulk of the card\'s content.',
              'image' => '...',
              'button_text' => 'Go somewhere',
              'button_url' => '#'
            ),
            array(
              'name' => 'Faculty of Computing',
              'description' => 'Some quick example text to build on the card title and make up the bulk of the card\'s content.',
              'image' => '...',
              'button_text' => 'Go somewhere',
              'button_url' => '#'
            ),
            array(
              'name' => 'Faculty of Engineering',
              'description' => 'Some quick example text to build on the card title and make up the bulk of the card\'s content.',
              'image' => '...',
              'button_text' => 'Go somewhere',
              'button_url' => '#'
            ),
            array(
              'name' => 'Faculty of Medicine',
              'description' => 'Some quick example text to build on the card title and make up the bulk of the card\'s content.',
              'image' => '...',
              'button_text' => 'Go somewhere',
              'button_url' => '#'
            ),
            array(
              'name' => 'Faculty of Science',
              'description' => 'Some quick example text to build on the card title and make up the bulk of the card\'s content.',
              'image' => '...',
              'button_text' => 'Go somewhere',
              'button_url' => '#'
            ),
          );

          foreach ($faculties as $faculty) {
        ?>
        <div class="col-4">
          <div class="card">
            <img src="<?php echo $faculty['image']; ?>" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title"><?php echo $faculty['name']; ?></h5>
              <p class="card-text"><?php echo $faculty['description']; ?></p>
              <a href="<?php echo $faculty['button_url']; ?>" class="btn btn-primary"><?php echo $faculty['button_text']; ?></a>
            </div>
          </div>
        </div>
        <?php } ?>
        
      </div>
    </div>
  </header>
</div>


    
</body>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN" crossorigin="anonymous">
</script>
</body>
</html>